...
export class RemoveTeamMemberComponent implements OnInit {
  ...
  Remove() {
    for (var i = 0; i < this.accounts.length; i++) {
      ...
      if (
        this.accounts[i].firstName == this.nam &&
        this.accounts[i].lastName == this.nam1
      ) {
    ...
    if (c) {
      this.freeServ
        .removeTeamMember(this.member.teamId, this.idToRemove)
        .subscribe((a) => {
          console.log('removed');
          ...
          for (var i = 0; i < this.accounts.length; i++) {
            for (var y = 0; y < this.freelancersIds.length; y++) {
              if (this.freelancersIds[y] == this.accounts[i].id) {
                this.mName =
                  this.accounts[i].firstName + ' ' + this.accounts[i].lastName;
